...
use App\Notification\Sender\MattermostChannelSender;
...
final class MattermostChannelSenderTest extends TestCase
{
    public function testSendsMarkdownSummaryToConfiguredTransport(): void
    {
        $sent = [];
        ...
        $channel = new NotificationChannel('mattermost', ChannelType::Mattermost, null, null, 'mattermost://token@example.test/?channel=securite');
        $result = (new MattermostChannelSender($resolver, $this->createTwig()))->send($channel, [$this->createMatch()]);

        self::assertSame(1, $result->sent());
        self::assertStringContainsString('SFR', $sent[0]->getSubject());
    }

    public function testRecordsFailureWhenChannelIsMissing(): void
    {
        $resolver = $this->createMock(TransportResolverInterface::class);
        $resolver->expects(self::never())->method('resolve');

        $channel = new NotificationChannel('mattermost', ChannelType::Mattermost, null, null, 'mattermost://token@example.test/');
        $result = (new MattermostChannelSender($resolver, $this->createTwig()))->send($channel, [$this->createMatch()]);
